document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchCarouselData() {
    try {
      const response = await fetch('assets/js/carousel-data.json');
  ...
  async function fetchCodeContent(filePath) {
    try {
      ...
      if (!response.ok) {
        console.error(`Failed to fetch code from: ${filePath}`);
        return '';
      }
  ...
  async function createCarouselCards(data) {
    ...
    cards = await Promise.all(data.map(async (item, index) => {
      console.log('Current Item:', item); // ADD THIS LINE
      ...
      technologiesElement.textContent = 'Technologies: ' + item.technologies.join(', ');
      ...
      try {
        ...
        if (!response.ok) {
          console.error(`Failed to fetch code from: ${item.code_file} - Status: ${response.status}`);
          codeText = `Error loading code from ${item.code_file} (Status: ${response.status})`;
        } else {
          codeText = await response.text();
          console.log(`Code fetched successfully from: ${item.code_file}`, codeText);
          card.dataset.code = codeText;
          const codeElement = document.createElement('code');
          // Assuming you'll handle Prism.js elsewhere or don't need it in this specific version
          codeElement.textContent = codeText;
          codeBlock.appendChild(codeElement);
          // If you need Prism here, add the language class and highlighting call
        }
      } catch (error) {
  ...
  function initializeCarousel() {
    fetchCarouselData().then(data => {
      createCarouselCards(data).then(() => { // Ensure cards are created before styling
  ...
  function handleResize() {
    ...
    if (currentWidth === 601) {
      ...
      carousel.style.transform = `translateY(35%) translateX(0%) rotateY(80deg)`; // Adjust these values as needed
